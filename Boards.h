...
#define TOTAL_PINS              39 // 29 digital +  4 led + 6 analog
#ifdef MAX_SERVOS
#undef MAX_SERVOS
#define MAX_SERVOS              NUM_DIGITAL_PINS // All pins can be servo with SoftPWMservo
#endif
#define VERSION_BLINK_PIN       LED_BUILTIN
#define IS_PIN_DIGITAL(p)       ((p) <= 32)
#define IS_PIN_ANALOG(p)        ((p) >= 33 && (p) <= 38)
#define IS_PIN_PWM(p)           ((p) == 6 || (p) == 5 || (p) == 9 || (p) == 3) // digitalPinHasPWM(p)
#define IS_PIN_SERVO(p)         ((p) <= 28)
#define IS_PIN_I2C(p)           ((p) == SDA || (p) == SCL)
#define IS_PIN_SPI(p)           ((p) == 10 || (p) == 11 || (p) == 12 || (p) == 13)
#define PIN_TO_DIGITAL(p)       (((p) < 29) ? (p) : (_LED_PIN((p) - 29)))
#define PIN_TO_ANALOG(p)        ((p) - 33)
#define PIN_TO_PWM(p)           (p)
#define PIN_TO_SERVO(p)         (p)
